...
if [[ -d $FRONTEND ]]; then
  ...
  # ARQUITECTURA: frontend usa SEMPRE o porto 9000 (dashboard server).
  # O dashboard faz proxy:
  #   /api/* → http://localhost:8000/*  (nexus REST API interna)
  #   /ws    → ws://localhost:8000/ws   (nexus WebSocket interno)
  # Assim apenas o porto 9000 precisa de estar aberto no firewall.
  cat > "$FRONTEND/.env" <<EOF
# Gerado por install.sh em $(date)
VITE_API_URL=http://${SERVER_IP}:9000/api
VITE_WS_URL=ws://${SERVER_IP}:9000/ws
EOF
...
#   nexus-api:       uvicorn nexus.api.rest.main:app em :8000 (REST + WS /ws)
#   nexus-dashboard: uvicorn nexus.dashboard.server:app em :9000 (frontend + proxy)
#
# nexus-core (python -m nexus.main) foi REMOVIDO — tinha conflito de WS thread
# nexus-ws   (ws_server.py em :8801) é opcional — não usado pelo frontend via proxy
...
if command -v ufw &>/dev/null; then
  ...
  ok "ufw: ports 8000 and 9000 open"
fi
...
echo -e "  API proxy : ${BLUE}http://$SERVER_IP:9000/api/health${NC}"
echo -e "  API direct: ${BLUE}http://$SERVER_IP:8000/docs${NC}"
...
echo -e "Depois: sudo systemctl restart nexus-api nexus-dashboard\n"
