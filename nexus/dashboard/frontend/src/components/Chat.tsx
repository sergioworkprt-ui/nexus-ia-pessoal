...
export default function Chat({ onAvatarState, ws }: Props) {
  ...
  return (
    // flex-col + flex-1 + min-h-0: permite que a área de mensagens faça
    // overflow-y:auto sem empurrar o input para fora do ecrã.
    <div className="flex flex-col flex-1 min-h-0">
      ...
      {/* Área de mensagens: flex-1 + min-h-0 + overflow-y-auto */}
      <div className="flex-1 min-h-0 overflow-y-auto p-4 space-y-3">
        {msgs.length === 0 && (
          <p className="text-center text-gray-600 text-sm mt-16">Olá! Como posso ajudar?</p>
        )}
        {msgs.map(m => (
          <div key={m.ts} className={`flex ${
            m.role === 'user' ? 'justify-end' : 'justify-start'
          } gap-2`}>
            {m.role === 'nexus' && (
              <div className="mt-1 shrink-0"><Avatar state="idle" size="sm" /></div>
            )}
          ...
          <div className="flex justify-start gap-2">
            ...
            <div className="bg-nexus-panel border border-nexus-border px-4 py-2 rounded-2xl rounded-tl-sm">
              <span className="inline-flex gap-1">
                {[0,1,2].map(i => (
                  <span key={i} className="w-1.5 h-1.5 bg-nexus-accent rounded-full animate-bounce"
      ...
      {/* Input: sempre fixo na base, não empurrado pelo conteúdo */}
      <div className="p-3 border-t border-nexus-border flex gap-2 shrink-0">
        ...
        <input
          className="flex-1 bg-nexus-bg border border-nexus-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-nexus-accent min-w-0"
