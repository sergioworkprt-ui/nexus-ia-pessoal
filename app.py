...
def xtb_account():
    ...
    session_data, err = xtb_login(
        ...
        os.environ.get('XTB_PASSWORD',''), mode
    )
    if err:
...
def xtb_order():
    """
    Executa ordem real — REQUER autorização nível 3 completa.
    WebAuthn + PIN + frase "AUTORIZO EXECUÇÃO REAL"
    """
    data = request.json or {}
    message = data.get('auth_phrase', '')
    action = f"ordem {data.get('cmd','?')} {data.get('amount_eur','?')}€ {data.get('symbol','?')}"

    # Verificação nível 3 completa
    auth = check_action_allowed(action, dict(session), message)
    if not auth['allowed']:
        audit_log(session['user_id'], 'XTB_ORDER_BLOCKED', action, DB_PATH, level=3,
                  result='blocked', metadata=auth)
        return jsonify({'error': auth['reason'], 'needs': auth['needs']}), 403
    ...
    audit_log(session['user_id'], 'XTB_ORDER', action, DB_PATH, level=3,
