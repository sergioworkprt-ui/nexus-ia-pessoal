...
def get_ai_response(messages, memory_context=''):
    ...
    configured = [name for name, fn in providers
                  if os.environ.get(name.split()[0].upper() + '_API_KEY', '')
                  or name == 'OpenRouter' and os.environ.get('OPENROUTER_API_KEY')]
    msg = (
        "⚠️ Nenhuma IA disponível de momento.\n\n"
        f"Erros:\n" + "\n".join(f"• {e}" for e in errors[:4]) +
        "\n\n💡 **Soluções:**\n"
        "• Verifica se as API Keys estão corretas no Render → Environment\n"
        "• Gemini 503 = sobrecarga temporária, tenta de novo em 1 min\n"
        "• Groq 403 = chave expirada, cria nova em console.groq.com\n"
        "• Os comandos internos (mostra limites, etc.) funcionam sem IA"
    )
    return msg, "none"
