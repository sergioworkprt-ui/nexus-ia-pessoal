...
def xtb_login(account_id, password, mode='demo'):
    """
    Login XTB. Usa modo 'demo' por padrão para segurança.
    Para real: mode='real' + autorização nível 3 obrigatória.
    """
    import os
    account_id = account_id or os.environ.get('XTB_ACCOUNT_ID', '')
    password = password or os.environ.get('XTB_PASSWORD', '')
    if not account_id or not password:
        return None, "XTB_ACCOUNT_ID e XTB_PASSWORD não configurados"
    payload = {
        "command": "login",
        "arguments": {
            "userId": account_id,
    ...
    # XTB usa WebSocket mas tem endpoints REST limitados
    # Para MVP completo usar a biblioteca python-xapi ou websockets
    result, err = _xtb_request("/login", payload)
    if err:
        return None, f"Login XTB falhou: {err}"
    if result and result.get('status'):
        token = result.get('streamSessionId', '')
        logger.info(f"XTB login OK — modo: {mode}")
        return {'token': token, 'mode': mode, 'account': account_id}, None
    return None, f"XTB login rejeitado: {result}"
